...
class _RequestBoardScreenState extends State<RequestBoardScreen> with SingleTickerProviderStateMixin {
    ...
    if (requests.isEmpty) {
      return Center(
        child: Column(
          ...
          children: [
            Icon(Icons.inbox_outlined, size: 64.sp, color: Colors.grey.shade400),
            SizedBox(height: 16.h),
            Text('No requests found', style: TextStyle(fontSize: 18.sp, color: Colors.grey.shade600)),
          ],
        ),
      );
    }
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    Container(
                      ...
                      decoration: BoxDecoration(color: _getTypeColor(request.materialType).withOpacity(0.1), borderRadius: BorderRadius.circular(10.r)),
                      child: Icon(_getTypeIcon(request.materialType), color: _getTypeColor(request.materialType), size: 22.sp),
                    ),
                    SizedBox(width: 12.w),
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(request.title, style: TextStyle(fontSize: 16.sp, fontWeight: FontWeight.bold, color: Colors.grey.shade800)),
                          SizedBox(height: 2.h),
                          Text('${request.materialType} • ${request.quantity}', style: TextStyle(fontSize: 13.sp, color: Colors.grey.shade600)),
                        ],
                      ),
                    ),
                ...
                Container(
                  ...
                  child: Row(
                    children: [
                      Icon(Icons.rocket_launch_outlined, size: 16.sp, color: Colors.grey.shade600),
                      SizedBox(width: 8.w),
                      Expanded(child: Text(request.project, style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.w500, color: Colors.grey.shade700))),
                    ],
                  ),
                ),
                SizedBox(height: 12.h),
                Row(
                  children: [
                    CircleAvatar(radius: 12.r, backgroundColor: Theme.of(context).colorScheme.primaryContainer, child: Text(request.requester[0], style: TextStyle(fontSize: 10.sp, fontWeight: FontWeight.bold))),
                    SizedBox(width: 8.w),
                    Text(request.requester, style: TextStyle(fontSize: 12.sp, color: Colors.grey.shade600)),
                    const Spacer(),
                    Icon(Icons.calendar_today_outlined, size: 14.sp, color: Colors.grey.shade500),
                    SizedBox(width: 4.w),
                    Text(_formatDeadline(request.deadline), style: TextStyle(fontSize: 12.sp, color: _isOverdue(request.deadline) ? Colors.red : Colors.grey.shade600)),
                  ],
                ),
                  ...
                  Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Text('Match Progress', style: TextStyle(fontSize: 12.sp, color: Colors.grey.shade600)),
                          Text('${request.matchedPercentage}%', style: TextStyle(fontSize: 12.sp, fontWeight: FontWeight.bold, color: Theme.of(context).colorScheme.primary)),
                        ],
                      ),
                      SizedBox(height: 4.h),
                      LinearProgressIndicator(value: request.matchedPercentage! / 100, backgroundColor: Colors.grey.shade200, borderRadius: BorderRadius.circular(4.r)),
                    ],
                  ),
                ],
  ...
  Widget _buildUrgencyBadge(RequestUrgency urgency) {
    ...
    return Container(
      ...
      child: Text(text, style: TextStyle(color: color, fontSize: 11.sp, fontWeight: FontWeight.w600)),
    );
  }
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              TextField(decoration: InputDecoration(labelText: 'Material Name', hintText: 'e.g., Arduino Uno Board', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r)))),
              SizedBox(height: 12.h),
              Row(
                children: [
                  Expanded(child: TextField(decoration: InputDecoration(labelText: 'Quantity', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r))))),
                  SizedBox(width: 12.w),
                  Expanded(child: DropdownButtonFormField<String>(decoration: InputDecoration(labelText: 'Type', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r))), items: ['Electronic', 'Metal', 'Plastic', 'Other'].map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(), onChanged: (v) {})),
                ],
              ),
              SizedBox(height: 12.h),
              TextField(decoration: InputDecoration(labelText: 'Project Name', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r)))),
              SizedBox(height: 12.h),
              TextField(maxLines: 2, decoration: InputDecoration(labelText: 'Description', hintText: 'Describe your material requirements...', border: OutlineInputBorder(borderRadius: BorderRadius.circular(12.r)))),
              SizedBox(height: 20.h),
              SizedBox(width: double.infinity, child: ElevatedButton(onPressed: () { Navigator.pop(context); }, style: ElevatedButton.styleFrom(padding: EdgeInsets.symmetric(vertical: 16.h)), child: const Text('Post Request'))),
              SizedBox(height: 16.h),
            ],
          ),
        ),
